...
def gen_mesh(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        ...
        for v in range(image_tensor_global.shape[0]):
            save_img = (np.transpose(image_tensor_global[v].detach(
            ).cpu().numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :, ::-1] * 255.0
        ...
        for i in range(len(color) // interval + 1):
            ...
            if i == len(color) // interval:
                ...
                right = (i + 1) * interval
            net.calc_normal(
                verts_tensor[:, None, :, left:right], calib_tensor[:, None], calib_tensor)
...
def gen_mesh_imgColor(res, net, cuda, data, save_path, thresh=0.5, use_octree=True, components=False):
    ...
    try:
        ...
        if 'calib_world' in data:
            calib_world = data['calib_world'].numpy()[0]
            verts = np.matmul(np.concatenate(
                [verts, np.ones_like(verts[:, :1])], 1), inv(calib_world).T)[:, :3]
...
def recon(opt, use_rect=False):
    ...
    if opt.load_netMR_checkpoint_path is not None:
        ...
        state_dict_path = '%s/%s_train_latest' % (
            opt.checkpoints_path, opt.name)
        opt.resume_epoch = 0
    else:
        state_dict_path = '%s/%s_train_epoch_%d' % (
            opt.checkpoints_path, opt.name, opt.resume_epoch)
    ...
    state_dict = torch.load(
        'C:/Users/Seif Yasser/Desktop/Project DEPI/PIFuHD/pifuhd/checkpoints/pifuhd.pt', map_location=cuda)
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_id, end_id)):
            ...
            if True:
                ...
                print(save_path)
                gen_mesh(opt.resolution, netMR, cuda, test_data,
                         save_path, components=opt.use_compose)
            else:
